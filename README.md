...
> **项目定位：Codex 原生像素风，而不是写实桌宠。** 与已有的写实或插画风奶蛙桌宠不同，本项目不是把写实形象简单缩小，而是按照 Codex 原版宠物的小尺寸像素表现、紧凑轮廓、状态动画和透明图集规范重新制作，放在 Codex 界面中更协调。

> 本项目为社区制作的非官方 Codex 宠物，与 OpenAI 无隶属或背书关系。
...
这里的“符合 Codex 原版像素风”指视觉语言和技术规格相协调，不表示本项目是 OpenAI 官方内置宠物。
...
- 当前 Codex 桌面版点击浮动宠物时，只会把主窗口带到前台并聚焦输入框，不会播放额外动画。
- 鼠标悬停由 Codex 固定映射到第 4 行，因此本项目把该行设计为五帧站地捧腹大笑，而不是跳跃。
- Codex 更新后若宠物没有自动刷新，请在“设置 → Pets”中点击刷新，或重启 Codex。
...
- “奶蛙”的网络流行形象原型不属于本项目所能确认或授予的权利范围。本项目仅就维护者实际创作的像素重绘、动画编排与文件组织授予许可，不对原型的权利状态作保证。
